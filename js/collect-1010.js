var WebCollectConfig = WebCollectConfig || {"appid": "1010", "appkey": "8e1d9C033dA9f2bA22F11165D9C7E"};
(function () {
  ...
  WebCollectConfig.callback = function (callbckObj) {
    ...
    if (WebCollectConfig.getServerFlag) {
      ...
      if (WebCollectConfig.requestList.length > 0) {
        for (var k = 0, len = WebCollectConfig.requestList.length; k < len; k += 1) {
  ...
  function fireRequest(business, data, callback, option_server, extendArg) {
    var consArgs = {_t: getTimestamp(), business: business, appid: WebCollectConfig.appid}, requestLink = "",
      web_collectLink = "http://webcollect.kugou.com", rt_webcollectLink = "http://rt.webcollect.kugou.com";
  ...
  var Md5 = {
    hex_chr: "0123456789abcdef", rhex: function (num) {
      var str = "";
      for (var j = 0; j <= 3; j += 1) {
  ...
  function CrossDomainGet(url) {
    var script = document.createElement("script"), DynamicId = Math.random().toString().split(".").pop();
    script.async = "async";
    script.type = "text/javascript";
    script.src = url;
    script.id = DynamicId;
  ...
  function dealPara(para, data) {
    ...
    iarr.sort();
    for (var k = 0, len = iarr.length; k < len; k += 1) {
  ...
  function init(business, datas, callback, option_server, extendArg) {
    try {
      if (apmCollectData && apmCollectData.length > 0) {
        for (var j = 0, len = apmCollectData.length; j < len; j += 1) {
  ...
  try {
    if (apmCollectData && apmCollectData.length > 0) {
      for (var k = 0, len = apmCollectData.length; k < len; k += 1) {
